...
Iclean = im2bw(I,lvl);
imshow(Iclean);             %show binary image
...
sig_clusters=zeros(num+1);
av_clstr_size=round(mean(count_array(:,1)));
j=1;
for i=1:1:num-1
if(count_array(i+1,1)>av_clstr_size)
sig_clusters(j)=i; 
j=j+1;
end
end

Im_sig_clusters=Cluster_Label_Im;              %will contain binary image of significant clusters

elim_list=zeros(num);               
for x=1:1:num
elim_list(x)=x;
end
elim_list = setxor(elim_list,sig_clusters);    %list of non-significant clusters   

%Deleting elements belonging to non-significant clusters below
for x=1:1:size(elim_list)
Im_sig_clusters(Im_sig_clusters==elim_list(x))=0;
end

figure, imshow(Im_sig_clusters);                       %show significant clusters only
