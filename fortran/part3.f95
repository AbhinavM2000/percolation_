...
function count_paths(matrix2,s1,s2) result(sum) !sum is being returned
implicit none
   integer:: i, path_finder,sum,matrix2(s1,s2),s1,s2
   sum=0
   do i = 1,s1
...
write(1,*)"Site occupation probability, N(paths), Array size | Step size : ",x
write(1,*)"--------------------------------------------------------------------"
close(1)
do inp2=0.93,1.00,x
p=0
...
p=p+count_paths(darray,s1,s2)	!calling the function to count paths
enddo
p=p/2
OPEN(UNIT=1,FILE="data1.txt",position='append',FORM="FORMATTED",STATUS="OLD")  !open textfile
write(1,*)inp2,"	",p
...
print*, "Data sucessfuly written to data1.txt" 
end do
PAUSE
end program path
